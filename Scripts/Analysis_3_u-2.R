...
### Two rate categories: -4 and -2 meanlog. Run for all four simulated datasets.
# This simulation method generates whole matrices using the sim.morpho() feature from the 'DispRity' package.

#### This script generates matrices with meanlog = -2. See other script for -4.
...
av_matrices <- c()
for(i in 1:length(av_n_subsets)){
	av_matrices[[i]] <- lapply(av_sub_trees[[i]], sim.morpho, characters=av_char, states=c(0.9, 0.1), model="ER", rates=c(rlnorm, meanlog=-2, sdlog=0.1), invariant=FALSE)
	print(i)
	}

# Generate 100 matrices at that the transition rate for the full set of taxa
av_matrices_ <- replicate(100, sim.morpho(av_tree, characters=av_char, states=c(0.9, 0.1), model="ER", rates=c(rlnorm, meanlog=-2, sdlog=0.1), invariant=FALSE))
...
pdf(file="Analysis3_meanlog-2_Avialae_2024-02-20.pdf")
...
max(1-ci_med) - min(1-ci_med)

max(1-ri_med) - min(1-ri_med)

max(1-rhi_med) - min(1-rhi_med)
...
neo_matrices <- c()
for(i in 1:length(neo_n_subsets)){
	neo_matrices[[i]] <- lapply(neo_sub_trees[[i]], sim.morpho, characters=neo_char, states=c(0.9, 0.1), model="ER", rates=c(rlnorm, meanlog=-2, sdlog=0.1), invariant=FALSE)
	print(i)
	}
	
# Generate 100 matrices at that the transition rate for the full set of taxa
neo_matrices_ <- replicate(100, sim.morpho(neo_tree, characters=neo_char, states=c(0.9, 0.1), model="ER", rates=c(rlnorm, meanlog=-2, sdlog=0.1), invariant=FALSE))
...
pdf(file="Analysis3_meanlog-2_Neornithes_2024-02-20.pdf")
...
pas_matrices <- c()
for(i in 1:length(pas_n_subsets)){
	pas_matrices[[i]] <- lapply(pas_sub_trees[[i]], sim.morpho, characters=pas_char, states=c(0.9, 0.1), model="ER", rates=c(rlnorm, meanlog=-2, sdlog=0.1), invariant=FALSE)
	print(i)
	}
	
# Generate 100 matrices at that the transition rate for the full set of taxa
pas_matrices_ <- replicate(100, sim.morpho(pas_tree, characters=pas_char, states=c(0.9, 0.1), model="ER", rates=c(rlnorm, meanlog=-2, sdlog=0.1), invariant=FALSE))
...
pdf(file="Analysis3_meanlog-2_Passeriformes_2024-02-20.pdf")
...
a <- max(1-ci_med) - min(1-ci_med)

b <- max(1-ri_med) - min(1-ri_med)

c <- max(1-rhi_med) - min(1-rhi_med)
...
tel_matrices <- c()
for(i in 1:length(tel_n_subsets)){
	tel_matrices[[i]] <- lapply(tel_sub_trees[[i]], sim.morpho, characters=tel_char, states=c(0.9, 0.1), model="ER", rates=c(rlnorm, meanlog=-2, sdlog=0.1), invariant=FALSE)
	print(i)
	}
	
# Generate 100 matrices at that the transition rate for the full set of taxa
tel_matrices_ <- replicate(100, sim.morpho(tel_tree, characters=tel_char, states=c(0.9, 0.1), model="ER", rates=c(rlnorm, meanlog=-2, sdlog=0.1), invariant=FALSE))
...
pdf(file="Analysis3_meanlog-2_Telluraves_2024-02-20.pdf")
...
d <- max(1-ci_med) - min(1-ci_med)

e <- max(1-ri_med) - min(1-ri_med)

f <- max(1-rhi_med) - min(1-rhi_med)
