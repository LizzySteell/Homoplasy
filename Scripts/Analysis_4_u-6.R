...
for(i in 1:length(av_chars)){
	av_matrices[[i]] <- replicate(100, sim.morpho(av_tree, characters=av_chars[[i]], states=c(0.9, 0.1), model="ER", rates=c(rlnorm, meanlog=-6, sdlog=0.1), invariant=FALSE), simplify=FALSE)
	print(i)
	}
...
pdf(file="Analysis4_meanlog-6_Avialae_2024-02-26.pdf")
...
for(i in 1:length(neo_chars)){
	neo_matrices[[i]] <- replicate(100, sim.morpho(neo_tree, characters=neo_chars[[i]], states=c(0.9, 0.1), model="ER", rates=c(rlnorm, meanlog=-6, sdlog=0.1), invariant=FALSE), simplify=FALSE)
	print(i)
	}
...
pdf(file="Analysis4_meanlog-6_Neornithes_2024-02-26.pdf")
...
for(i in 1:length(pas_chars)){
	pas_matrices[[i]] <- replicate(100, sim.morpho(pas_tree, characters=pas_chars[[i]], states=c(0.9, 0.1), model="ER", rates=c(rlnorm, meanlog=-6, sdlog=0.1), invariant=FALSE), simplify=FALSE)
	print(i)
	}
...
pdf(file="Analysis4_meanlog-6_Passeriformes_2024-02-26.pdf")
...
for(i in 1:length(tel_chars)){
	tel_matrices[[i]] <- replicate(100, sim.morpho(tel_tree, characters=tel_chars[[i]], states=c(0.9, 0.1), model="ER", rates=c(rlnorm, meanlog=-6, sdlog=0.1), invariant=FALSE), simplify=FALSE)
	print(i)
	}
...
pdf(file="Analysis4_meanlog-6_Telluraves_2024-02-26.pdf")
